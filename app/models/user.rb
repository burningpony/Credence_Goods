  ...
  class Part2
    ...
    def payment
      if(@user.is_expert?)
        ...
        return STARTING_CASH - expert.function_responses.where(part: :part_2).sum(&:point_difference)
      end
    end
    ...
    def num_uniq_group_responses
      function_responses.distinct.pluck(:function_set_id).join(:function).where("max_value_prediction IS NOT NULL").count
    end
